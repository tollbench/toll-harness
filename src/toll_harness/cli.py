...
_FILE_INFORMED_PLAN_INSTRUCTION = (
    ...
    "outcome_promise names the thing, never you (\"An email draft ready for your "
    "approval.\"); its `you` line says what the person does (\"Approve the email to "
    "<who>\"); the person approves the email word for word and Book of Houses sends "
    "it; its single review_approve block is the person confirming the send receipt. "
    "For a want that arranges a "
    ...
    "`plan_examples`: worked programs, each a COMPLETE proposal that already passes the validate "
    "door, and `wants_like` says which wants each is for. Do not compose a plan out of parts. "
    "`nearest_program` on the brief is the pick, inline, with one sentence in `program_to_copy`: "
    "copy its `proposal` WHOLE, change only what this want makes different -- the words, the "
    "recipient, the numbers -- keep its shape (its steps, its acts, its connect_account rows, its "
    "question formats), validate, file once. A program's work is a `calls` act: runs of name, "
    "tool, row, args, each, wait -- where `tool` is a registry verb, composio:<slug>/<TOOL_SLUG>, "
    "key:<slug>/<action>, mcp:<server>/<tool> or platform.notify/draft/contact/research, `row` "
    "names the `connect_account` row on that same step, `each` runs the call once per picked "
    "contact, and `wait` carries event, of and timeout_hours. SAY WHERE EVERY ARGUMENT CAME FROM: "
    "it is a literal you wrote, or a declared $from source -- person.<question id>, an EARLIER "
    "run[.field], or draft.<name>. There is no fourth source, a run may not quote a run that has "
    "not happened, a tool needs its row on the step that runs it, and a recipient is never a "
    "literal address (REJ-41). If the person answered the contact question with a research brief "
    "(`contact_research` on the brief), bind the send to a research run -- the recipient reads "
    "$from find.contact -- or set contact_from research on a legacy email act; never add a picker "
    "they declined and never invent an address. "
